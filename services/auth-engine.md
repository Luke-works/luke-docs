...
- **CORS ahead of every gate** — the gateway terminates CORS, and its `CorsFilter` is explicitly ordered ahead of the security chain and the rate limiter (just behind the correlation-id filter) so a **401 or 429 is still CORS-readable**. Registered as a plain filter bean it would land at `LOWEST_PRECEDENCE` — behind the security chain — and every 401 for an aged-out access token would reach the browser without `Access-Control-Allow-Origin`, surfacing as an intermittent "CORS error"/"Failed to fetch" and defeating the UI's refresh-on-401 retry. Pinned by `CorsConfigTest` plus a real-HTTP assertion in `ProxyAuthFunctionalTest`.
...
- **Upstream circuit breaker** — the proxy tracks consecutive failures (timeouts, connection errors, upstream 5xx) **per upstream** (core-engine / file-proxy). After a threshold it OPENS and fails fast with `503` + `Retry-After` — no upstream call — for a cooldown, so a downstream outage can't pile every caller up behind the full timeout and drag the gateway down with it; one probe then closes it on success or re-opens on failure. Default-lenient: only trips on repeated *real* failures (a single blip resets on the next success), state is per-upstream (a sick core never trips file-proxy), and it's disableable (`luke.auth.proxy.circuit-breaker.enabled=false`). Tunable via `failure-threshold` / `open-seconds`.
