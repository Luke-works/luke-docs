...
| **usage** | `usage` | Monthly **usage metering** — `UsageService` counts consumption into `luke_usage_counter` (one row per `tenant\|metric\|YYYY-MM`); today it meters submissions and emails at their choke points. Recording is best-effort in a `REQUIRES_NEW` tx and swallows all errors, so it can never break a metered op. Self-serve `GET /api/usage` returns used-vs-limit. Enforcement (`402` over the cap) is **opt-in** via `luke.plan.enforce-usage-limits` (default off). See [Plans, Limits & Usage](/concepts/plans#metering-what-a-tenant-actually-used) |
