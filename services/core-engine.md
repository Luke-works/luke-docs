...
| **branding** | `branding` | Per-tenant commercial plan (`luke_tenant_plan`, FREE\|PAID — absent row = FREE) and `BrandingPolicy`, the single home for the "Developed at Lukeflow" badge rule on public form surfaces. `PlanFeatures` gates the other paid features (file attachments). Operator-only `PUT /api/tenants/{id}/plan` is the seam real billing will write to. See [Forms](/capabilities/forms#developed-at-lukeflow-badge) and [File attachments](/capabilities/forms#file-attachments-paid) |
