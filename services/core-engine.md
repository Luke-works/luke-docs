...
| **branding** | `branding` | Per-tenant commercial **plan tier** — `PlanCatalog` (the SSOT enum: `FREE / PRO / BUSINESS / ENTERPRISE`, each carrying its price, monthly limits and entitlements inline; absent/unknown → FREE, legacy `PAID` → PRO) — plus `BrandingPolicy`, the single home for the "Developed at Lukeflow" badge rule on public form surfaces. Self-serve `GET /api/plan`; operator `PUT /api/tenants/{id}/plan` is the seam real billing writes to. See [Plans, Limits & Usage](/concepts/plans), [Forms](/capabilities/forms#developed-at-lukeflow-badge) and [File attachments](/capabilities/forms#file-attachments-paid) |
| **usage** | `usage` | Monthly **usage metering** — `UsageService` counts consumption into `luke_usage_counter` (one row per `tenant\|metric\|YYYY-MM`); today it meters submissions and emails at their choke points. Recording is best-effort in a `REQUIRES_NEW` tx and swallows all errors, so it can never break a metered op. It also folds in a live **storage gauge** — a SUM of the tenant's non-deleted `luke_document` bytes via a `StorageUsageProvider` SPI (not a counter — storage is occupied, not accumulated). Self-serve `GET /api/usage` returns used-vs-limit (storage in bytes). Enforcement (`402` over the cap) is **opt-in** via `luke.plan.enforce-usage-limits` (default off). See [Plans, Limits & Usage](/concepts/plans#metering-what-a-tenant-actually-used) |
| **billing** | `billing` | **Stripe checkout** — `StripeBillingService` mints a hosted Checkout Session (`POST /api/billing/checkout`) and applies the result via a signature-verified webhook (`POST /webhooks/stripe`, mounted outside `/api/**`) through `PlanService.applyPlan`; the browser never sets a plan. `luke_billing_customer` maps Stripe customer↔tenant. **Config-gated** on `STRIPE_SECRET_KEY` — unset = the module self-disables (checkout 404s, webhook no-ops), so dev/qa never touch Stripe. Price ids are env placeholders (`STRIPE_PRICE_PRO` / `STRIPE_PRICE_BUSINESS`). See [Plans → Billing](/concepts/plans#billing-stripe) |
| **payments** | `payments` | **Form payments** — bring-your-own Stripe via Connect OAuth, **direct charges** on the tenant's account, no fee. `PaymentAmountResolver` (Java mirror of form-core's pricing, parity-tested) prices a submission at the `FormSubmissionService` choke point through the `PaymentGate` seam; the instance waits in `AWAITING_PAYMENT` until `FormPaymentService.settle` sees Stripe report `succeeded` (payer sync, `POST /webhooks/stripe-connect`, or the `PaymentReconciler`, which runs on its own thread under a time budget and cancels abandoned charges at Stripe before releasing them). A charge the platform loses access to ends `UNRESOLVED` (audited). Disconnect cancels open charges first and never revokes a grant another workspace still uses; the tenant purge cancels and revokes after commit. Owner-only settings at `/api/payments/**`. Tables `luke_payment_account`, `luke_form_payment`, `luke_payment_connect_state`, `luke_payment_webhook_event` (V26). **Config-gated** on the secret + publishable keys + Connect client id. See [Forms → Payments](/capabilities/forms#payments-stripe-bring-your-own-account) |
| **ai** | `ai` | **AI provider (bring your own key)** — each workspace connects **as many LLM accounts as it likes** (Groq, OpenAI, Anthropic, Gemini) and picks a model from any of them; Lukeflow pays nothing for inference, so there is **no plan gate**, only an owner check. Each provider gets its own key, stored in `luke_secrets` under `ai.provider-key.<provider>` (AES-256-GCM, the same store Postmark and Vapi use), and its own `luke_ai_provider` row keyed `<tenantId>:<provider>` (V27, made multi-row by **V29** — before that a second provider silently overwrote the first one's key). The row holds only the provider, model, status, the key's last four characters and a SHA-256 fingerprint. Exactly one row is `preferred` — the provider a turn runs on absent a personal choice — and `AiProviderService` demotes it on every path that can make it unservable (marked invalid, verified invalid, secret missing, disconnected), so a workspace cannot end up with two defaults. A person's own model choice lives in `luke_ai_user_pref` (V28) and may name a model from any connected provider. `AiProviderProbe` verifies a key with the provider **before** it is stored and returns the models that key may actually use. `AgentProxyController` is the **only** door to [Agents](/services/agents): `POST /api/ai/agents/{slug}/{op}` authenticates the caller, checks the mapped capability, and attaches the decrypted key server-side — the browser never holds it and nothing of the caller's request is forwarded but the JSON body. "The provider refused this key" (→ `INVALID`) is kept strictly apart from "we couldn't reach the provider" (→ no change), so a provider blip can't disconnect a working workspace. Settings at `/api/ai/**`. **Config-gated** on `LUKE_AI_AGENTS_URL` |
