...
| **ai** | `ai` | **AI provider (bring your own key)** — each workspace connects its own LLM account (Groq, OpenAI, Anthropic, Gemini) and picks a model; Lukeflow pays nothing for inference, so there is **no plan gate**, only an owner check. The key is stored in `luke_secrets` under `ai.provider-key` (AES-256-GCM, the same store Postmark and Vapi use); `luke_ai_provider` (V27) holds only the provider, model, status, the key's last four characters and a SHA-256 fingerprint. `AiProviderProbe` verifies a key with the provider **before** it is stored and returns the models that key may actually use. `AgentProxyController` is the **only** door to [Agents](/services/agents): `POST /api/ai/agents/{slug}/{op}` authenticates the caller, checks the mapped capability, and attaches the decrypted key server-side — the browser never holds it and nothing of the caller's request is forwarded but the JSON body. "The provider refused this key" (→ `INVALID`) is kept strictly apart from "we couldn't reach the provider" (→ no change), so a provider blip can't disconnect a working workspace. Settings at `/api/ai/**`. **Config-gated** on `LUKE_AI_AGENTS_URL` |
