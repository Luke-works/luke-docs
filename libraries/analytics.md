...
- **Now consumed:** `analytics-core` + `analytics-react` are **vendored into** the [Consumer UI](/apps/consumer-ui), which renders them at `/analytics` — a submission dashboard (volume over time, busiest forms, state mix, average turnaround) driven by real engine queries over the tenant's form instances. Nivo arrives with it as a peer dep, so the route is lazy-loaded into its own chunk. `analytics-builder` is **not** vendored yet: it ships bare `analytics-*` class names and needs a theme bridge first, like `lukeforms-theme.css`.
- **Young:** the repo is a **scaffold on local `main`** (not pushed); expect the public API surface to still move. Two contract details bit the first consumer and are worth knowing: `Query.sort` is a **single** `SortSpec`, not an array, and `DataRow` values are `ScalarValue` — so "no value" is `null` (which `avg` skips), never `undefined`.
