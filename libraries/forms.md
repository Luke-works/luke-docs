...
- **The field-type registry is published as a fixture.** `npm run fixtures:field-types` writes
  `fixtures/field-types.json` from `createDefaultFieldTypeRegistry()`. luke-agents copies it and
  fails a test when a type is in neither its supported list nor a documented exclusion — the
  reverse direction of `agent-schema-cases.json`, which travels the other way. It exists because
  the form agent silently fell to 13 of 48 types and told a user the builder had no tabs, panels
  or stepper while the palette showed all three.
