...
|---------|------|----------------|
| `@lukeflow/form-core` | The headless engine — schema contract, dependency-graph + topological settlement, validation registry, field-type registry, the safe expression parser and the JS logic layer, the Minions data-source contract, the pure builder operations, and the design-time **data-contract** derivation. No React, no DOM. | `createFormEngine`, `validateSchema` / `repairSchema` / `migrateSchema`, builder ops, expression + JS evaluators, data-source helpers, `deriveDataContract`, `buildFormTemplate` |
...
- **Data Structure view** — a design-time `Design ⇄ Data` toggle in the builder that shows the data contract a form produces: every field's submission key, value type (`string` / `number` / `string[]` / `row[]` / `object` …), how it accepts input, and its constraints, beside an example submission payload. Backed by the pure `deriveDataContract(schema)` in form-core, which mirrors the runtime submission shape (grids nest as `row[]`, layout containers flatten, `addressBlock` expands, `persistent:false` excluded) — so what it shows is exactly what an integration receives. It also offers **Generate template** — a pure `buildFormTemplate(schema)` derives a spreadsheet template (one column per fillable field; `addressBlock` expands, repeating grids reported separately) and a ready-to-use **CSV** (UTF-8 BOM, opens in Excel) to prefill and run against the form (e.g. an outbound campaign); the `FormBuilder` `onGenerateTemplate` prop lets a host emit a real `.xlsx` instead.
...
- **Every value is checked for KIND and DOMAIN, not just constraints.** Each declarative rule (`min`, `maxLength`, `pattern`…) only applies once the author sets its attribute, so a field with no constraints used to accept anything: a `number` stored `"abc"`, and a `select` accepted a value its author never offered. Two base rules close that — `type` (the value must parse as the kind the field holds; blanks are `required`'s business and bounds are `min`/`max`'s) and `option` (a choice must be one on offer, inert for minion-backed fields whose options aren't in the schema). Both are enforced in **both** implementations and covered by the shared parity fixture.
- **A checkbox parses strings rather than trusting truthiness.** `Boolean(str)` is true for every non-empty string, so a posted `"no"` or `"false"` recorded consent. Only an explicit true token (`true`/`1`/`on`/`yes`/`y`/`checked`) reads as checked — which matters wherever a caller posts strings instead of booleans (an embed, a CSV import, an API client).
...
- **Payments** — a `payment` field priced `fixed`, `perUnit` (price × a payer quantity) or `entered` (payer amount with bounds), never from a formula. form-core ships the server-authoritative `resolvePaymentAmount`, exact ISO-4217 minor-unit arithmetic and ten payment diagnostics, including `payment-wizard-page`, `payment-unreachable` and currency/precision checks on the amount field. The Java copy shares `fixtures/payment-parity.json` (167 cases). form-react adds a processor-neutral `PaymentSession` / `PaymentsProvider` (with an optional `pricingSchema` for hosts that render a locked copy; the card form counts as loaded only once Stripe reports it ready, a later load failure marks the session unavailable, and an unwanted mount is aborted), the payment control (offline preview · live card form · read-only charge status), a `submitting` prop that blocks double submits, and the opt-in **`@lukeflow/form-react/stripe`** subpath (Payment Element, deferred intent, card-only; Stripe.js from `js.stripe.com` only, a bundled copy is refused). The builder offers the field (hideable with `hiddenFields`) with money and source-field editors, and hides every standard editor that would make a payment conditional, computed or excluded. See [Forms → Payments](/capabilities/forms#payments-stripe-bring-your-own-account).
...
| E2E | Playwright across Chromium / Firefox / WebKit (incl. a **layout-invariant matrix** — jsdom has no layout engine, so box overflow, stretch, grid placement and reading order are only assertable in a real browser) |
| Visual regression | Pixel baselines on Chromium/Linux, gated behind `VISUAL=1`; regenerated by the `Visual baselines` workflow |
...
visual:        pixel baselines (Chromium/Linux) — pushes to main only, not PRs
...
- **QuickJS is opt-in at the library level** — the default `Function` evaluator is explicitly *not* a security boundary. It **is** wired in consumer-ui, whose renderer defaults `allowJs` to `false` and whose fill path opts in with a lazily-loaded, memoized isolate (the single-file embedded-WASM variant, so there is no separate `.wasm` asset to serve or 404); until it loads, author JS stays off rather than falling back. The builder package's own in-modal preview still uses the default evaluator — the author running their own code in their own browser.
- **Validation is mirrored server-side, not shared.** The declarative rules are re-implemented in Java in core-engine so a tampered payload can't skip them, kept honest by `fixtures/validation-parity.json` — a 91-case table executed against both implementations, with a revision tripwire that fails the Java build if the two copies drift. Expressions, author JS, and async/minion validation stay client-only by design.
