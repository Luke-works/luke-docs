...
- **Data view annotations** (`dataAnnotations`) — a host-defined choice offered against every data variable in the builder's Data view, with the field list grouped by the answer. Deliberately domain-free: the library knows only that there are named options, that one may carry a **qualifier** nested under another option (so a host with three stored states can still present two answers), and how to bucket rows by the result. `hideDataView` removes the Design/Data toggle entirely for forms where the contract tells the author nothing actionable, and falls back to the canvas if a host hides it while it is open. `dataMetadata` lists always-recorded rows as their own card. Lukeflow drives [outbound fill roles](/capabilities/forms#data-view-outbound-only) through all three.
...
- **Validation is mirrored server-side, not shared.** The declarative rules are re-implemented in Java in core-engine so a tampered payload can't skip them, kept honest by `fixtures/validation-parity.json` — a case table executed against both implementations. Expressions, author JS, and async/minion validation stay client-only by design.
